...
{
	FormValidator(sForm, sErrorBox, false);
}


function FormValidator(sForm, sErrorBox, bMoveToField)
{
	if (typeof bMoveToField === "undefined")
		bMoveToField = false;
	...
	this.bMoveToField  = bMoveToField;
}
	...
	for (var i = 0; i < sCheckOptions.length; i ++)
	{
		switch(sCheckOptions[i])
		{
			case "B" : if (trim(this.objForm[eField].value) == "")
			           {
			           	if (this.sErrorBox != "")
			           		showMessage(this.sErrorBox, "alert", sMsg, this.bMoveToField)
...
function showMessage(sDivId, sClass, sMessage, bMoveToField)
{
	if (typeof bMoveToField === "undefined")
		bMoveToField = false;
