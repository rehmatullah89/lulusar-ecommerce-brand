...
?>
  <script type="text/javascript" src="scripts/<?= $sCurDir ?>/social-media.js?<?= @filemtime("{$sAdminDir}scripts/{$sCurDir}/social-media.js") ?>"></script>
</head>
...
?>
	  ...
	  <div class="dataGrid ex_highlight_row">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblData" id="DataGrid">
		  ...
		  <tbody>
<?
	$sSQL = "SELECT * FROM tbl_social_media ORDER BY id";
	$objDb->query($sSQL);
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$sProfileUrl = $objDb->getField($i, "profile_url");
		$sLogin      = $objDb->getField($i, "login");
