...
?>
	    ...
	    <div id="tabs-1">
		  ...
		  <div class="dataGrid ex_highlight_row">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tblData" id="DataGrid" rel="tbl_blog_categories">
			  <thead>
			    <tr>
			      <th width="5%">#</th>
			      <th width="35%">Category Name</th>
			      <th width="30%">SEF URL</th>
			  ...
			  <tbody>
<?
	$sSQL = "SELECT * FROM tbl_blog_categories ORDER BY position";
	$objDb->query($sSQL);
...
?>
		        <tr id="<?= $iId ?>">
		          ...
		          <td><?= ((($sParent != '') ? "{$sParent} &raquo; " : "").$sName) ?></td>
		          <td><?= $sSefUrl ?></td>
...
<?
		...
		if ($sPicture != "" && @file_exists($sRootDir.BLOG_CATEGORIES_IMG_DIR.$sPicture))
		{
?>
					<img class="icnPicture" id="<?= (SITE_URL.BLOG_CATEGORIES_IMG_DIR.$sPicture) ?>" src="images/icons/picture.png" alt="Picture" title="Picture" />
<?
		}
?>
...
<?
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			...
			$sSefUrl = $objDb->getField($i, "sef_url");
?>
			    	  <option value="<?= $iParent ?>" sefUrl="<?= $sSefUrl ?>"<?= ((IO::intValue('ddParent') == $iParent) ? ' selected' : '') ?>><?= $sParent ?></option>
<?
		}
?>
				    </select>
