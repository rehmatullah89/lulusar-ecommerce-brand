...
?>
  <script type="text/javascript" src="scripts/<?= $sCurDir ?>/swithdrawals.js"></script>
</head>
...
<?
                            if($_POST)
                            {
                                $iItemCount = getDbValue("COUNT(1)", "tbl_stocks", "code='$sBarCode' AND status = 'A'");
...
?>
		  ...
		  <input type="hidden" id="TotalRecords" value="<?= $iTotalRecords = getDbValue('COUNT(1)', 'tbl_stocks_history') ?>" />
...
<?
	...
	{
		$sSQL = "SELECT *,
                                (SELECT name from tbl_admins WHERE id=tbl_stocks_history.modified_by) as _ModifiedBy
