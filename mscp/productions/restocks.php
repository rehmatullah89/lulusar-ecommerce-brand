<?
	...
	***********************************************************************************************
	...
	$objDb2      = new Database( );
        ...
        $StockHistDetailIds = IO::getArray("StockHistDetailId");
        
	if ($_POST["Save"] != "" && !empty($StockHistDetailIds))
		@include("save-restock.php");  
...
?>
  <script type="text/javascript" src="scripts/<?= $sCurDir ?>/restocks.js"></script>
</head>
...
<?
	...
	{
?>
            <li><a href="<?= $_SERVER['REQUEST_URI'] ?>#tabs-1"><b>Re-Stock</b></a></li>
            
            <li><a href="<?= $_SERVER['REQUEST_URI'] ?>#tabs-2">Re-Stocks History</a></li>
...
<?
                            if($_POST)
                            {
                                $iItemCount = getDbValue("COUNT(1)", "tbl_stocks", "code='$sBarCode' AND status = 'I'");
...
<?
                                        ...
                                        for ($i = 0; $i < $iCount; $i ++)
                                        {
                                            ...
                                            $iStockId    = getDbValue("id", "tbl_stocks", "code LIKE '$sCode'");
?>
                                            <tr id="<?=$sCode?>">
                                                <td><?=$i+1?><input type="hidden" id="StockHistDetailId" name="StockHistDetailId[]" value="<?=$iStockId?>"></td>                                            
                                                <td><?=$sProductName?></td>
...
<?
                            ...
                            {
?>
                              <button id="BtnSave" name="Save" value="Save">Save Restock</button>
<?
                            }
?>
                              <button id="BtnCancel2">Back</button>
                            </td>
<?
                            }
...
?>
		  ...
		  <input type="hidden" id="TotalRecords" value="<?= $iTotalRecords = getDbValue('COUNT(1)', 'tbl_restocks') ?>" />
...
<?
	...
	{
		$sSQL = "SELECT *,
                                (SELECT name from tbl_admins WHERE id=tbl_restocks.modified_by) as _ModifiedBy
				 FROM tbl_restocks";
		$objDb->query($sSQL);
		...
		{
			$iId        = $objDb->getField($i, "id");
			$iItems     = $objDb->getField($i, "restock_ids");
