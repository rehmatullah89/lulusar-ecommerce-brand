...
$(document).ready(function( )
{
	...
	$("#txtName, #ddParent, #txtSefUrl").blur(function( )
	{
		...
		$.post("ajax/catalog/check-category.php",
			{ SefUrl:sUrl },
	...
	$("#frmRecord").submit(function( )
	{
		...
		if (objFV.isChecked("cbFeatured"))
		{
			if (!objFV.validate("fileFeaturedPic", "B", "Please select the Category Picture (Home Page)."))
				return false;
		}

		
		if (objFV.value("fileFeaturedPic") != "")
		{
			if (!checkImage(objFV.value("fileFeaturedPic")))
			{
				showMessage("#RecordMsg", "alert", "Invalid File Format. Please select an image file of type jpg, gif or png.");

				objFV.focus("fileFeaturedPic");
				objFV.select("fileFeaturedPic");
	...
	if (parseInt($("#TotalRecords").val( )) > 100)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/catalog/get-categories.php",
										   ...
										   fnInitComplete  : function( )
															 {
																$.post("ajax/catalog/get-category-filters.php",
																	   {},
		...
		if (sCategories != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
																		{
																			 $.post("ajax/catalog/delete-category.php",
	...
	$(document).on("click", ".icnEdit", function(event)
	{
		...
		$.colorbox({ href:("catalog/edit-category.php?CategoryId=" + iCategoryId + "&Index=" + iIndex), width:"85%", height:"85%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("catalog/view-category.php?CategoryId=" + iCategoryId), width:"85%", height:"85%", iframe:true, opacity:"0.50", overlayClose:true });
	...
	$(document).on("click", ".icnToggle", function(event)
	{
		...
		$.post("ajax/catalog/toggle-category-status.php",
	...
	$(document).on("click", ".icnFeatured", function(event)
	{
		...
		$.post("ajax/catalog/toggle-category-featured-status.php",
			{ CategoryId:objIcon.id },
