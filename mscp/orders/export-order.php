<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT si.*,
                (SELECT name from tbl_countries where id=si.country_id) as ShippingCountry
                 FROM tbl_order_shipping_info si WHERE si.order_id='$iOrderId'";
	$objDb->query($sSQL);
	...
	$iShippingCountry   = $objDb->getField(0, "country_id");
        $sShippingCountry   = $objDb->getField(0, "ShippingCountry");
	...
	$sShippingEmail     = $objDb->getField(0, "email");
        ...
        if($iShippingCountry == '222')
            $sShippingFields = ($sShippingState.", ".$sShippingCountry);
        else if($iShippingCountry == '223')
            $sShippingFields = ($sShippingCity.", ".$sShippingZip.", ".$sShippingCountry);
        else if($iShippingCountry == '224' || $iShippingCountry == '38')
            $sShippingFields = ($sShippingCity.", ".$sShippingState.", ".$sShippingZip.", ".$sShippingCountry);
        else
            $sShippingFields = ($sShippingCity.", ".$sShippingCountry);
        


	$sSQL = "SELECT bi.*,
                (SELECT name from tbl_countries where id=bi.country_id) as BillingCountry
                FROM tbl_order_billing_info bi WHERE bi.order_id='$iOrderId'";
	$objDb->query($sSQL);
	...
	$iBillingCountry   = $objDb->getField(0, "country_id");
        $sBillingCountry   = $objDb->getField(0, "BillingCountry");
	...
	$sBillingEmail     = $objDb->getField(0, "email");
        ...
        if($iBillingCountry == '222')
            $sBillingFields = ($sBillingState.", ".$sBillingCountry);
        else if($iBillingCountry == '223')
            $sBillingFields = ($sBillingCity.", ".$sBillingZip.", ".$sBillingCountry);
        else if($iBillingCountry == '224' || $iBillingCountry == '38')
            $sBillingFields = ($sBillingCity.", ".$sBillingState.", ".$sBillingZip.", ".$sBillingCountry);
        else
            $sBillingFields = ($sBillingCity.", ".$sBillingCountry);
	...
	$objPdf->Text($iLeft, ($iTop + 13), $sBillingName);
        ...
        $objPdf->SetXY($iLeft-1, ($iTop + 13.5));
	$objPdf->MultiCell(70, 2.5, $sBillingAddress, 0, 'L', false);	
	$objPdf->Text($iLeft, ($iTop + 21), $sBillingFields);

        $objPdf->SetFont('Arial', '', 7);
	$objPdf->Text(($iLeft + 80), ($iTop + 13), $sShippingName);
        
        $objPdf->SetFont('Arial', '', 6);
        $objPdf->SetXY(($iLeft + 79), ($iTop + 13.5));
	$objPdf->MultiCell(70, 2.5, $sShippingAddress, 0, 'L', false);	
        $objPdf->Text(($iLeft + 80), ($iTop + 21), $sShippingFields);
