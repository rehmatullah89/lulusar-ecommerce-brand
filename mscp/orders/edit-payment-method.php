<?
	...
	***********************************************************************************************
	...
	if ($_POST)
		@include("save-payment-method.php");
...
?>
  <script type="text/javascript" src="scripts/<?= $sCurDir ?>/edit-payment-method.js?<?= @filemtime("{$sAdminDir}scripts/{$sCurDir}/edit-payment-method.js") ?>"></script>
</head>
...
<?
	...
	{
		if ($iMethodId == 11)
			$sSupportedCurrencies = "'AUD','BHD','GBP','BGN','CAD','HRK','CZK','DKK','EUR','HKD','HUF','ISK','INR','ILS','JPY','JOD','KWD','LTL','MYR','MAD','TRY','NZD','NOK','OMR','PLN','QAR','RON','SAR','RSD','SGD','ZAR','KRW','SEK','CHF','TWD','THB','TND','USD','AED'";
?>
		  <label for="txtBusinessEmail">Business Email</label>
		  <div><input type="text" name="txtBusinessEmail" id="txtBusinessEmail" value="<?= $sBusinessEmail ?>" maxlength="100" size="40" class="textbox" /></div>
<?
...
?>
		  ...
		  <label for="filePicture">Logo <span><?= (($sPicture == "") ? '' : ('('.substr($sPicture, ((@strpos($sPicture, "{$iMethodId}-") === FALSE) ? 0 : strlen("{$iMethodId}-"))).')')) ?></span></label>
		  <div><input type="file" name="filePicture" id="filePicture" value="" size="35" class="textbox" /></div>
		  ...
		  <br />
		  <button id="BtnSave">Save Payment Method</button>
		  <button id="BtnCancel">Cancel</button>
		</td>

		<td>
		  <label for="">Checkout Currencies <span>(<a href="#" rel="Check">Check All</a> | <a href="#" rel="Clear">Clear</a>)</span></label>
