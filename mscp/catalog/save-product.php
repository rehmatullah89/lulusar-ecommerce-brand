<?
	...
	***********************************************************************************************
	...
	$fPrice       	    = IO::floatValue("txtPrice47");
	...
	$iCurrencies        = IO::getArray("Currencies");
	$sRelatedProducts   = "";
	$sPicture           = "";
	$sPicture2          = "";
	$sPicture3          = "";
...
//		$iProduct = getNextId("tbl_products");
		...
		if ($bFlag == true)
		{
			for ($i = 0; $i < count($iCurrencies); $i ++)
			{
				$fCountryPrice  = IO::floatValue("txtPrice{$iCurrencies[$i]}");
				$sCountryStatus = IO::strValue("cbAvailable{$iCurrencies[$i]}");
				
				if ($sCountryStatus == "")
					$sCountryStatus = "I";


				$sSQL  = "INSERT INTO tbl_product_prices SET product_id  = '$iProduct',
															 currency_id = '{$iCurrencies[$i]}',
															 price       = '$fCountryPrice',
															 status      = '$sCountryStatus'";
				$bFlag = $objDb->execute($sSQL);
		...
		if ($bFlag == true)
		{
			for ($i = 0; $i < count($iOptions); $i ++)
			{
				...
				$sSQL  = ("INSERT INTO tbl_product_options SET product_id   = '$iProduct',
															   attribute_id = '0',
															   option_id    = '".intval($iOption[0])."',
															   option2_id   = '".intval($iOption[1])."',
															   option3_id   = '".intval($iOption[2])."',
															   price        = '0',
															   sku          = '',
															   `quantity`   = '$iQuantity'");
				$bFlag = $objDb->execute($sSQL);
