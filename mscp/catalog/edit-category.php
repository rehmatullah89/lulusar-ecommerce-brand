		...
		<td width="400">
		  ...
		  <div>
		    <select name="ddParent" id="ddParent">
			  <option value=""></option>
<?
	$sSQL = "SELECT id, name, sef_url FROM tbl_categories WHERE parent_id='0' AND id!='$iCategoryId' ORDER BY name";
	$objDb->query($sSQL);
...
?>
			  <option value="<?= $iParentId ?>" sefUrl="<?= $sUrl ?>"<?= (($iParent == $iParentId) ? ' selected' : '') ?>><?= $sParent ?></option>
<?
		...
		for ($j = 0; $j < $iCount2; $j ++)
		{
			...
			$sUrl        = $objDb2->getField($j, "sef_url");
?>
	    	  <option value="<?= $iParentId ?>" sefUrl="<?= $sUrl ?>"<?= (($iParent == $iParentId) ? ' selected' : '') ?>><?= ($sParent." &raquo; ".$sParentName) ?></option>
<?
		}
	}
?>
		  ...
		  <div>
		    ...
		    <input type="text" name="txtSefUrl" id="txtSefUrl" value="<?= (($iParent > 0) ? substr($sSefUrl, (strrpos(substr($sSefUrl, 0, -1), '/') + 1)) : $sSefUrl) ?>" maxlength="100" size="44" class="textbox" />
		  </div>

		  <div class="br10"></div>

		  <label for="filePicture">Picture <span><?= (($sPicture == "") ? '(optional)' : ('(<a href="'.(SITE_URL.CATEGORIES_IMG_DIR.'listing/'.$sPicture).'" class="colorbox">'.substr($sPicture, strlen("{$iCategoryId}-")).'</a> - <a href="'.$sCurDir.'/delete-category-picture.php?CategoryId='.$iCategoryId.'&Index='.$iIndex.'&Field=picture">Delete</a>)')) ?></span></label>
		  ...
		  <label for="fileFeaturedPic">Featured Pic <span><?= (($sFeaturedPic == "") ? '(optional)' : ('(<a href="'.(SITE_URL.CATEGORIES_IMG_DIR.'featured/'.$sFeaturedPic).'" class="colorbox">'.substr($sFeaturedPic, strlen("{$iCategoryId}-")).'</a> - <a href="'.$sCurDir.'/delete-category-picture.php?CategoryId='.$iCategoryId.'&Index='.$iIndex.'&Field=featured_pic">Delete</a>)')) ?></span></label>
		  <div><input type="file" name="fileFeaturedPic" id="fileFeaturedPic" value="" size="40" class="textbox" /></div>
