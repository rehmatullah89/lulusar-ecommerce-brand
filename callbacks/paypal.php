<?
	...
	***********************************************************************************************
	...
	$sBusinessEmail      = IO::strValue("business");
	$sReceiverEmail      = IO::strValue("receiver_email");
	$sCustomerEmail      = IO::strValue("payer_email");
	$iOrderTransactionId = IO::strValue("custom");


	if ($sBusinessEmail == $sMerchantEmail || $sReceiverEmail == $sMerchantEmail)
	{
		...
		else
		{
			...
			while (!@feof($hSocket))
			{
				...
				if (strcmp($sResult, "VERIFIED") == 0)
				{
					...
					else if (@in_array(strtolower($sPaymentStatus), array("failed", "denied", "expired", "voided")))
