<?
	...
	***********************************************************************************************
	...
	$sSQL = "SELECT * FROM tbl_banners WHERE FIND_IN_SET('F', placements) AND status='A' $sBannersSql ORDER BY position";
	$objDb->query($sSQL);
...
?>
			<div id="BannerF<?= $iId ?>" style="width:<?= $iWidth ?>px; height:<?= $iHeight ?>px; overflow:hidden; margin-top:15px;">
<?
		if ($sType == "F")
		{
?>
			  ...
			  <!--
				  ...
				  {
					  $("#BannerF<?= $iId ?>").flash({	src:'<?= (BANNERS_IMG_DIR.$sBanner) ?>', width:<?= $iWidth ?>, height:<?= $iHeight ?> });
				  });
			  -->
			  </script>
<?
