...
*/
		    	$this->_font=dirname(__FILE__)."/../../fonts/arialbd.ttf";
			}

			else
			{
/*
		    	$this->_font=dirname($_SERVER["PATH_TRANSLATED"])."/"."courbd.ttf";

				if (isset($_SERVER['WINDIR']) && file_exists($_SERVER['WINDIR']))
				  $this->_font=$_SERVER['WINDIR']."\Fonts\courbd.ttf";
*/
				  $this->_font=dirname(__FILE__)."/../../fonts/courbd.ttf";
			}
		...
		*/
		...
		function _c93Barcode($barnumber,$scale=1,$file="",$checkdigit=false)
		{
			...
			$x = ((int)$font_arr[2] - (int)$font_arr[0]);
			$x = (($total_x / 2) - ($x / 2));
		...
		function _c128Barcode($barnumber,$scale=1,$file="")
		{
			...
			if (@strpos($_SERVER['PHP_SELF'], "inventory/") !== FALSE)
				$font_arr=@imagettfbbox ( $scale*17, 0, $this->_font, $barnumber);
			...
			if (@strpos($_SERVER['PHP_SELF'], "inventory/") !== FALSE)
				@imagettftext($im,$scale*18,0,($x-20), $height2, $bar_color,$this->_font , $barnumber);
